...
def partition_balanced(nums: np.ndarray, k: int):
    ...
    maxval = np.full((n + 1, k + 1), dtype=np.int64, fill_value=np.iinfo(np.int64).min)
    minval = np.full((n + 1, k + 1), dtype=np.int64, fill_value=np.iinfo(np.int64).max)
    split = [[[0]] * (k + 1) for _ in range(n + 1)]

    for i in range(n + 1):
        dp[i, 1] = 0
        maxval[i, 1] = sum(nums[:i])
        minval[i, 1] = sum(nums[:i])

    for j in range(2, k + 1):
        for i in range(j, n + 1):
            for x in range(1, i):
                min_diff = max(dp[x, j - 1], maxval[x, j - 1] - sum(nums[x:i]),
                               sum(nums[x:i]) - minval[x, j - 1])
                dp[i, j] = min(dp[i, j], min_diff)

                if dp[i, j] == min_diff:
                    split[i][j] = split[x][j - 1] + [x]
                    if dp[i, j] == maxval[x, j - 1] - sum(nums[x:i]):
                        maxval[i, j] = maxval[x, j - 1]
                        minval[i, j] = sum(nums[x:i])
                    elif dp[i, j] == sum(nums[x:i]) - minval[x, j - 1]:
                        maxval[i, j] = sum(nums[x:i])

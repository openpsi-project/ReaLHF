...
@numba.njit
def _partition_balanced(nums: np.ndarray, k: int):
    ...
    # s1 = [[0 for _ in range(k)] for _ in range(n + 1)]
    # s2 = [[0] for _ in range(n + 1)]
    s1 = np.full((n + 1, k), dtype=np.int64, fill_value=0)
    s2 = np.full((n + 1), dtype=np.int64, fill_value=0)

    # split = [[0 for _ in range(k)] for _ in range(n + 1)]
    ...
    for j in range(2, k + 1):
        ...
        for i in range(n + 1, 0, -1):
            s1[i] = np.copy(s1[s2[i]])
            s1[i, j - 1] = s2[i]
    return np.concatenate((s1[n], np.array([n])), axis=0)


def partition_balanced(nums: np.ndarray, k: int):
    return list(_partition_balanced(nums, k))


def partition_balanced_tuples(nums: np.ndarray, k: int):
    lst = partition_balanced(nums, k)
    return [(lst[i], lst[i + 1]) for i in range(k)]


@numba.njit
def _min_abs_diff_partition(arr: np.ndarray, k: int) -> List[Tuple[int]]:
...
def min_abs_diff_partition(arr: Union[np.ndarray, List], k: int) -> List[Tuple[int]]:
    ...
    partitions = partition_balanced_tuples(arr, k)
    last_end = 0
...
if __name__ == "__main__":
    ...
    for i in range(1, 3):
        st = time.monotonic()
        nums = np.array(list(range(1, i * 4000)))
        res = partition_balanced(nums, 4)

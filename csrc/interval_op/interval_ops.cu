...
__global__ void set_intervals_kernel(
    const scalar_t *src,
    scalar_t *dst,
    const long *interval_sizes,
    ...
    int num_intervals)
{
    ...
    {
        if (tid * chunk_size + i >= interval_size)
        {
            break;
        }
...
template <typename scalar_t, int chunk_size>
void set_intervals_cuda(
    const torch::Tensor &src,
    const torch::Tensor &dst,
    const torch::Tensor &intervals,
    ...
    long max_interval_size)
{
    ...
    set_intervals_kernel<scalar_t, chunk_size>
        <<<numBlocks, threadsPerBlock>>>(
            src.data_ptr<scalar_t>(),
            dst.data_ptr<scalar_t>(),
            interval_sizes.data_ptr<long>(),
            intervals.data_ptr<long>(),
...
template <typename scalar_t, int chunk_size>
__global__ void slice_intervals_kernel(
...
template <typename scalar_t, int chunk_size>
torch::Tensor slice_intervals_cuda(
    ...
    long output_size)
{
    ...
    slice_intervals_kernel<scalar_t, chunk_size>
        <<<numBlocks, threadsPerBlock>>>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def("set_intervals_cuda_half", &set_intervals_cuda<at::Half, 2048>, "Slice intervals CUDA (float16)");
    m.def("set_intervals_cuda_bfloat16", &set_intervals_cuda<at::BFloat16, 2048>, "Slice intervals CUDA (bfloat16)");
    m.def("slice_intervals_cuda", &slice_intervals_cuda<float, 2048>, "Slice intervals CUDA");
    m.def("slice_intervals_cuda_half", &slice_intervals_cuda<at::Half, 2048>, "Slice intervals CUDA (float16)");
    m.def("slice_intervals_cuda_bfloat16", &slice_intervals_cuda<at::BFloat16, 2048>, "Slice intervals CUDA (bfloat16)");
}
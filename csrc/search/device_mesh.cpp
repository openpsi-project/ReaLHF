...
DeviceMesh::DeviceMesh(std::string device_mesh_name, 
                       int n_nodes, 
                       int n_gpus, 
...
n_gpus_per_node(n_gpus_per_node) {
    ...
    for (std::string node_name : node_names) {
        this -> node_names.insert(node_name);
    }
    for (int gpu_id : gpu_ids) {
        this -> gpu_ids.insert(gpu_id);
    }
};
...
bool DeviceMesh::is_overlap(const DeviceMesh& other) {
    if (n_nodes > 1) {
        for (std::string node_name : node_names) {
            if (other.node_names.find(node_name) != other.node_names.end()) 
                return true;
        }
    } else if (n_nodes == 1) {
        if (other.node_names.find(*node_names.begin()) == other.node_names.end()) 
            return false;
        for (int gpu_id : gpu_ids) {
            if (other.gpu_ids.find(gpu_id) != other.gpu_ids.end()) 
                return true;
        }
    } 
    return false;
...
bool is_all_overlap(std::vector<DeviceMesh*> device_meshes, DeviceMesh device_mesh) {
    for (DeviceMesh* other : device_meshes) {
        if (!device_mesh.is_overlap(*other)) 
            return false;
    }
    return true;
};
...
bool DeviceMesh::is_contain(const DeviceMesh& other) {
    if (n_nodes > 1) {
        ...
        for (int gpu_id : other.gpu_ids) {
            if (gpu_ids.find(gpu_id) == gpu_ids.end()) 
                return false;
        }
    }
    return true;
...
std::vector<std::unordered_set<std::string>> divide(DeviceMesh& device_mesh, int n){
    ...
    if (n == 1) {
        ...
        if (device_mesh.n_nodes % n == 0){
            ...
            std::cout << "Divide fail: device mesh name " << device_mesh.device_mesh_name 
              << " n " << n << std::endl;
            assert(false);
        }
    }
...
std::string ModelParallelStrategy::to_string() {
    return "num_pp:" + std::to_string(num_pp) + ";" 
           + "num_dp:" + std::to_string(num_dp) + ";" 
           + "num_mp:" + std::to_string(num_mp);
};

std::string ModelParallelStrategy::to_key() {
    return std::to_string(num_pp) + "," 
           + std::to_string(num_mp) + ","
           + std::to_string(num_dp);
}
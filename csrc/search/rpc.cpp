...
RPCExecution::RPCExecution(RPC* rpc_ptr, 
...
bool OverlapGroup::maybe_add(RPCExecution* rpc_exe) {
    ...
    if (is_all_overlap(device_meshes, rpc_exe -> device_mesh)) {
        ...
        device_meshes.insert(&rpc_exe -> device_mesh);
        mem_static += rpc_exe -> static_mem;
        mem_active = std::max(mem_active, 
                              rpc_exe -> mem - rpc_exe -> static_mem);
        return true;
    }
    return false;
};
...
void DeviceMeshGroup::add_to_groups(RPCExecution* rpc_exe) {
    ...
    for (OverlapGroup* og : overlap_groups) {
        ...
        if (!update) {
            ...
            for (RPCExecution* rpc_exe : og -> rpc_executions) {
                tmp_new_ogs.back() -> maybe_add(rpc_exe);
            }
        }
    ...
    for (OverlapGroup* og : tmp_new_ogs) {
        overlap_groups.push_back(og);
    }
};
...
void GroupedRPCExecutions::add(RPCExecution* rpc_exe) {
    group.add_to_groups(rpc_exe);
};
...
uint64_t GroupedRPCExecutions::total_mem_cost() {
    ...
    for (auto& og : group.overlap_groups) {
        ...
        if (og -> mem_active + og -> mem_static > max_mem) {
            max_mem = og -> mem_active + og -> mem_static;
        }
    }
    return max_mem;
};
...
void RPCInstance::add_parent(RPCInstance* parent) {
    parents.push_back(parent);
};

void RPCInstance::add_child(RPCInstance* child) {
...
void RPCInstance::remove_tmp_child(RPCInstance* child) {
    ...
    if (it != tmp_children.end()) {
        tmp_children.erase(it);
    }
};

void RPCInstance::remove_tmp_parent(RPCInstance* parent) {
    auto it = std::find(tmp_parents.begin(), tmp_parents.end(), parent);
    if (it != tmp_parents.end()) {
        tmp_parents.erase(it);
    }
};

void RPCInstance::add_tmp_parent(RPCInstance* parent) {
    tmp_parents.push_back(parent);
};

void RPCInstance::add_tmp_child(RPCInstance* child) {
    tmp_children.push_back(child);
};

uint64_t parameter_sync_cost(uint64_t model_size, 
                             RPCExecution* src, RPCExecution* dst,
...
void RPCInstance::resolve_parameter_sync(std::vector<RPCInstance*> tmp_graph, 
    ...
    // dst to train
    uint64_t from_cost = parameter_sync_cost(param_sync_size, param_sync_rpc_exe_ptr, 
                                             rpc_exe_ptr, cost_table);
    uint64_t to_cost = parameter_sync_cost(param_sync_size, rpc_exe_ptr, 
                                           param_sync_rpc_exe_ptr, cost_table);
    ...
    RPCExecution* from_src_exe = new RPCExecution(rpc_ptr, 
                                    param_sync_rpc_exe_ptr -> device_mesh, 
                                    param_sync_rpc_exe_ptr -> model_parallel_strategy,
                                    from_cost, 0, 0);
    RPCInstance* from_src = new RPCInstance(rpc_ptr, id, name + ":from_src");
    from_src -> rpc_exe_ptr = from_src_exe;
    
    RPCExecution* from_dst_exe = new RPCExecution(rpc_ptr, 
    ...
    RPCExecution* to_src_exe = new RPCExecution(rpc_ptr, 
                                             rpc_exe_ptr -> device_mesh, 
                                             rpc_exe_ptr -> model_parallel_strategy,
                                             to_cost, 0, 0);
    ...
    RPCExecution* to_dst_exe = new RPCExecution(rpc_ptr, 
                                    param_sync_rpc_exe_ptr -> device_mesh, 
                                    param_sync_rpc_exe_ptr -> model_parallel_strategy,
                                    to_cost, 0, 0);
...
//     //           << offload_store_cost << " offload load cost " << offload_load_cost << std::endl;
...
CommStats::CommStats(uint64_t local_send, uint64_t local_recv, 
                     uint64_t remote_send, uint64_t remote_recv,
                     uint64_t offload_store, uint64_t offload_load)
: local_send(local_send), local_recv(local_recv), 
  remote_send(remote_send), remote_recv(remote_recv),
  offload_store(offload_store), offload_load(offload_load){
};

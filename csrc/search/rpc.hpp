...
class CommStats {
public:
    uint64_t local_send, local_recv, remote_send, remote_recv,
             offload_store, offload_load;

    CommStats(uint64_t local_send, uint64_t local_recv, 
...
public:
    ...
    RPCExecution(RPC* rpc_ptr, 
                 DeviceMesh& device_mesh, 
                 ModelParallelStrategy& model_parallel_strategy,
                 uint64_t time_cost, uint64_t mem, uint64_t static_mem);

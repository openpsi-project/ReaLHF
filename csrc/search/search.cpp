        ...
        std::sort(rpc_exe_list.begin(), rpc_exe_list.end(),
                  [](const RPCExecution *a, const RPCExecution *b)
                  {
                      if (a->time_cost == b->time_cost)
                          return a->device_mesh.device_mesh_name < b->device_mesh.device_mesh_name;
                      else
...
void make_sorted_rpc_names(std::vector<std::string> &sorted_rpc_names,
                           std::unordered_map<std::string, std::vector<RPCExecution *>> &rpc_exe_table)
{
    ...
    for (auto &x : rpc_exe_table)
    {
        ...
        average_time_cost.push_back(std::make_pair(rpc_name,
                                                   total_time_cost / 10));
    }
    ...
    for (auto &x : average_time_cost)
    {
        sorted_rpc_names.push_back(x.first);
    }
}
                                        ...
                                        MinEndTimeQueue &top_k_queue)
{
    ...
    prepare(rpc_exe_table, rpc_table, sorted_rpc_names, ri_table,
            model_name_ri_table, rpcs, rpc_exes, graph);
    ...
    // index 1
    SimulateResult first_sr = simulate(graph, cost_table, model_sizes,
                                       rpc_table, rpc_exe_table, ri_table,
                                       model_name_ri_table, sorted_rpc_names,
                                       index);
    ...
    while (valid_count < VALID_COUNT_CAP)
    {
        ...
        for (int i = 0; i < num_rpcs; i++)
        {
            ...
            int max_i = std::min(static_cast<int>(rpc_exe_table[rpc_name].size()),
                                 c_i + max_step_range + 1);
        ...
        SimulateResult selected_sr = simulate(graph, cost_table, model_sizes,
                                              rpc_table, rpc_exe_table, ri_table,
                                              model_name_ri_table, sorted_rpc_names,
                                              new_index);
            ...
            if (accepted)
            {
                ...
                if (!selected_sr.oom)
                {
                    ...
                    if (min_time_cost > selected_cost)
                    {
                        ...
                        double diff = std::chrono::duration_cast<std::chrono::milliseconds>(now - start).count();
                ...
                if (valid_count % 1000 == 0)
                {
                    std::cout << " valid_count " << valid_count
                              << " oom count " << oom_count
                              << " time " << diff.count()
                              << " min time cost " << min_time_cost
                              << " min index mem cost " << min_index_mem
                              << " max time cost " << max_time_cost
                              << " current cost " << current_cost
                              << " avg " << avg << " mem usage "
                              << check_memory_bytes() << std::endl;
    ...
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = end - start;
    std::cout << "MCMC Search finished, beta " << beta
    ...
    for (auto &re_ptr : final_sr.rpc_exe_list)
    {
...
)
{
    ...
    for (int i = 0; i < repeat; i++)
    {
        for (double beta = beta_min; beta < beta_max; beta += beta_step)
        {
            ...
            std::vector<SimulateResult>
                r = mcmc_search(rpcs, rpc_exes, graph, cost_table, model_sizes, beta, time_limit, *q);
        }
    }
                 ...
                 std::vector<RPCInstance *> graph, CommStats &comm_stats)
{
    ...
    for (auto &rpc_instance : graph)
    {
        ...
        for (auto &parent : rpc_instance->parents)
        {
            std::cout << parent->name << " ";
        }
        std::cout << std::endl;
        std::cout << "children" << std::endl;
        for (auto &child : rpc_instance->children)
        {
            std::cout << child->name << " ";
        }
        std::cout << std::endl;
...
{
    return new RPC(
        rpc_py.attr("model_name").cast<std::string>(),
        rpc_py.attr("rpc_name").cast<std::string>(),
        rpc_py.attr("interface_type").cast<std::string>());
}

DeviceMesh *cast_device_mesh(py::handle device_mesh_py,
                             std::unordered_map<std::string, DeviceMesh *> &device_mesh_map)
{
    std::string device_mesh_name = device_mesh_py.attr("device_mesh_name").cast<std::string>();
    if (device_mesh_map.find(device_mesh_name) == device_mesh_map.end())
    {
        DeviceMesh *device_mesh = new DeviceMesh(
            device_mesh_name,
            device_mesh_py.attr("n_nodes").cast<int>(),
            device_mesh_py.attr("n_gpus").cast<int>(),
            device_mesh_py.attr("node_names").cast<std::vector<std::string>>(),
            device_mesh_py.attr("gpu_ids").cast<std::vector<int>>(),
            device_mesh_py.attr("n_gpus_per_node").cast<int>());
        device_mesh_map[device_mesh_name] = device_mesh;
        return device_mesh;
    }
    else
    {
        return device_mesh_map[device_mesh_name];
    }
}
...
ModelParallelStrategy *cast_model_parallel_strategy(py::handle model_parallel_strategy_py)
{
    return new ModelParallelStrategy(
        model_parallel_strategy_py.attr("num_pp").cast<int>(),
        model_parallel_strategy_py.attr("num_dp").cast<int>(),
        model_parallel_strategy_py.attr("num_mp").cast<int>());
}

RPCExecution *cast_rpc_execution(py::handle rpc_exe_py,
                                 std::unordered_map<std::string, RPC *> &tmp,
                                 std::unordered_map<std::string, DeviceMesh *> &device_mesh_map)
{
    DeviceMesh *device_mesh = cast_device_mesh(rpc_exe_py.attr("device_mesh"),
                                               device_mesh_map);
    ...
    return new RPCExecution(
        tmp[rpc_exe_py.attr("rpc").attr("rpc_name").cast<std::string>()],
        *device_mesh,
        *model_parallel_strategy,
        rpc_exe_py.attr("time_cost").cast<uint64_t>(),
        rpc_exe_py.attr("mem").cast<uint64_t>(),
        rpc_exe_py.attr("static_mem").cast<uint64_t>());
}

RPCInstance *cast_rpc_instance_wo_dependency(py::handle rpc_instance_py,
                                             std::unordered_map<std::string, RPC *> &tmp)
{
    return new RPCInstance(
        tmp[rpc_instance_py.attr("rpc").attr("rpc_name").cast<std::string>()],
        rpc_instance_py.attr("epoch_id").cast<int>(),
        rpc_instance_py.attr("name").cast<std::string>());
}

void cast_rpc_instance_dependency(py::handle rpc_instance_py,
                                  RPCInstance *ri_ptr,
...
py::list py_single_mcmc_search_time_profile(
    ...
    py::object time_limit)
{
    ...
    std::unordered_map<std::string, uint64_t> cost_table = cost_table_py.cast<std::unordered_map<std::string, uint64_t>>();

    std::unordered_map<std::string, uint64_t> model_sizes = model_sizes_py.cast<std::unordered_map<std::string, uint64_t>>();
    MinEndTimeQueue res_queue(10);
    std::vector<SimulateResult> rlist = mcmc_search(
        rpcs, rpc_exes, graph, cost_table, model_sizes,
        beta.cast<double>(), time_limit.cast<double>(), res_queue);
    ...
    for (auto &r : rlist)
    {
        ...
        std::cout << std::endl;
        std::cout << "Time cost: " << r.end_time
                  << ", mem cost: " << r.mem_cost << std::endl;
        ...
        for (auto &re_ptr : r.rpc_exe_list)
        {
            ...
            rpc_exe_info["device_mesh"] = re_ptr->device_mesh.device_mesh_name;
...
py::list py_multi_mcmc_search(py::list rpcs_py, py::list rpc_exes_py,
                              py::list graph_py, py::dict cost_table_py,
                              py::dict model_sizes_py, py::object beta_min_py,
                              py::object beta_max_py, py::object beta_step_py,
                              py::object time_limit_py, py::object repeat)
{
    std::vector<RPC *> rpcs;
    std::unordered_map<std::string, RPC *> tmp;
    for (py::handle rpc_py : rpcs_py)
    ...
    MinEndTimeQueue res_queue(10);
    multi_mcmc_search(rpcs, rpc_exes, graph,
                      cost_table, model_sizes,
                      beta_min, beta_max,
                      beta_step, res_queue, 10, time_limit, rp);
...
PYBIND11_MODULE(mdm_search, m)
{
    ...
    m.def("input_check", [](py::list rpcs_py, py::list rpc_exes_py,
                            py::list graph_py, py::object comm_stats_py)
          {
    ...
    m.def("parameter_sync_cost", [](py::object rpcs_py,
                                    py::object param_size_bytes_py,
                                    py::dict cost_table_py,
                                    py::object src_py,
                                    py::object dst_py)
          {

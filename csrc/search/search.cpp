...
void prepare(std::unordered_map<std::string, std::vector<RPCExecution*>>& rpc_exe_table,
    ...
    for (auto& rpc_instance : graph) {
        ri_table[
            rpc_instance -> rpc_ptr -> rpc_name].push_back(rpc_instance);
    }
    
    for (auto& rpc_instance : graph) {
        model_name_ri_table[
            rpc_instance -> rpc_ptr -> model_name].push_back(rpc_instance);
    }
};
...
RPC* cast_rpc(py::handle rpc_py) {
    return new RPC(
        py::str(rpc_py.attr("model_name")).cast<std::string>(),
...
DeviceMesh* cast_device_mesh(py::handle device_mesh_py,
    ...
    if (device_mesh_map.find(name) == device_mesh_map.end()) {
        py::array_t<int32_t> mapping_array = device_mesh_py.attr("mapping")
            .cast<pybind11::array_t<int32_t>>();
...
RPCExecution* cast_rpc_execution(py::handle rpc_exe_py, 
    ...
    return new RPCExecution(
        tmp[rpc_exe_py.attr("rpc").attr("name").cast<std::string>()],
        *device_mesh,
...
RPCInstance* cast_rpc_instance_wo_dependency(py::handle rpc_instance_py, 
    ...
    return new RPCInstance(
        tmp[rpc_instance_py.attr("rpc").attr("name").cast<std::string>()],
...
void cast_rpc_instance_dependency(py::handle rpc_instance_py, 
    ...
    for (py::handle parent_py : rpc_instance_py.attr("parents"))
        ri_ptr -> parents.push_back(tmp_graph[
            parent_py.attr("name").cast<std::string>()]);
    for (py::handle child_py : rpc_instance_py.attr("children"))
        ri_ptr -> children.push_back(tmp_graph[
            child_py.attr("name").cast<std::string>()]);
}

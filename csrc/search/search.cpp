...
RPC *cast_rpc(py::handle rpc_py) {
  return new RPC(rpc_py.attr("model_name").cast<std::string>(),
...
DeviceMesh *cast_device_mesh(py::handle device_mesh_py,
  ...
  if (device_mesh_map.find(device_mesh_name) == device_mesh_map.end()) {
    DeviceMesh *device_mesh =
        new DeviceMesh(device_mesh_name, device_mesh_py.attr("n_nodes").cast<int>(),
...
ModelParallelStrategy *cast_model_parallel_strategy(py::handle model_parallel_strategy_py) {
  return new ModelParallelStrategy(model_parallel_strategy_py.attr("num_pp").cast<int>(),
...
RPCExecution *cast_rpc_execution(py::handle rpc_exe_py, std::unordered_map<std::string, RPC *> &tmp,
  ...
  return new RPCExecution(
      tmp[rpc_exe_py.attr("rpc").attr("rpc_name").cast<std::string>()], *device_mesh,
...
RPCInstance *cast_rpc_instance_wo_dependency(py::handle rpc_instance_py,
  ...
  return new RPCInstance(tmp[rpc_instance_py.attr("rpc").attr("rpc_name").cast<std::string>()],

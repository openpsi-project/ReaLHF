...
SimulateResult::SimulateResult(uint64_t end_time, bool oom,
                               uint64_t mem_cost, std::vector<int> &index)
    : end_time(end_time), oom(oom), mem_cost(mem_cost), index(index)
{
}

SimulateResult simulate(std::vector<RPCInstance *> &graph,
                        ...
                        std::vector<int> &index)
{
    ...
    for (int i = 0; i < num_rpcs; i++)
    {
        ...
        {
            if (rpc_instance->rpc_ptr->interface_type == "ModelInterfaceType.TRAIN_STEP" && rpc->interface_type != "ModelInterfaceType.TRAIN_STEP")
            {
    ...
    while (!ready_queue.empty())
    {
        ...
        {
            if (device_mesh->is_overlap(*mesh))
            {
                if (mesh->pre_task == nullptr ||
                    mesh->pre_task->end_time <= t->end_time)
                {
                    mesh->pre_task = t;
                }
                // mesh -> pre_task = t;
    ...
    for (RPCInstance *node : graph)
    {
        ...
        for (RPCInstance *ptr : node->tmp_ris)
        {
            delete ptr;
        }
        node->tmp_ris.clear();

        for (RPCExecution *ptr : node->tmp_exes)
        {
            delete ptr;
        }
        node->tmp_exes.clear();
    ...
    if (current_mem > SOFT_GPU_MEM_CAP)
    {
        max_end_time *= oom_penalty;
    }
    ...
    for (OverlapGroup *ptr : grouped_rpc_exe.group.overlap_groups)
    {
        delete ptr;
    }
    grouped_rpc_exe.group.overlap_groups.clear();

    return SimulateResult(max_end_time,
                          current_mem > SOFT_GPU_MEM_CAP,
                          current_mem,
                          index);
};
...
{
    std::priority_queue<SimulateResult, std::vector<SimulateResult>, CompareEndTime> pq = q.getQueue();
    std::queue<SimulateResult> tmp;
    while (!pq.empty())
    {
        if (pq.top().end_time == element.end_time)
        {
            return true;
        }
        tmp.push(pq.top());
...
void mergeMinEndTimeQueues(MinEndTimeQueue &target, MinEndTimeQueue &q1)
{
    ...
    std::priority_queue<SimulateResult, std::vector<SimulateResult>, CompareEndTime> pq1 = q1.getQueue();

    // Insert all elements from q1 into the merged queue
    while (!pq1.empty())
    {
        if (!isPresent(target, pq1.top()))
            target.insert(pq1.top());
        pq1.pop();
    }
}
...
public:
    ...
    SimulateResult(uint64_t end_time, bool oom, 
...
public:
    ...
    void insert(SimulateResult r) {
        if (queue.size() < k) {
            // std::cout << "push " << "end_time: " << r.end_time << " qsize " << queue.size() << std::endl;
            queue.push(r);
...
public:
    bool operator()(RPCInstance* r1, RPCInstance* r2) {
        return r1->ready_time > r2->ready_time;
    }
};

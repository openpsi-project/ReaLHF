...
which returns an :class:`realrlhf.NamedArray` object containing the data of a single sample and its sequence length.
...
- Implement a map-style PyTorch dataset class with a ``__getitem__`` method. This method returns an :class:`realrlhf.NamedArray` object containing the sequence length as metadata.
...
In ReaL, we implement a :class:`realrlhf.impl.model.nn.real_llm_api.ReaLModel` class that wraps the HuggingFace model and provides
...
A node is represented by a :class:`realrlhf.MFCDef` object.
...
.. autoclass:: realrlhf.ModelInterface
    :members:
    :undoc-members:
...
.. note::
    ...
    The ``SFTInterface`` is registered at the end of this file and constructed by :class:`realrlhf.SFTConfig`
    (see the ``rpcs`` method).
...
.. code-block:: console
    ...
    python3 -m realrlhf.apps.quickstart my-ppo \

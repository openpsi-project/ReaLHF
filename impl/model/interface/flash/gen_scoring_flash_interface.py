...
class PackedGenScoringInterface(api.model.ModelInterface):
    ...
    def __post_init__(self):
        ...
        self.sft_model = flash_mqat.HuggingfaceLikeFlashMQATForCausalLM.from_pretrained(
            model_path="/data/aigc/llm/checkpoints/fw/flash-ppo-s42/run20231106-rw2/actor@pp_00-mp_00-dp_00/epoch0step0/",
            dtype=torch.float16,
            device="cuda",
        )
        self.history_data = []
    ...
    def generate(self, model: api.model.Model, data: NamedArray) -> NamedArray:
        ...
        sft_model_logits = self.sft_model(
            input_ids=seq, attention_mask=attention_mask, padding_side=None
        ).logits
        sft_seqlogp = []

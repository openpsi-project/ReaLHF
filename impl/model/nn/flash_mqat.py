...
class CausalSelfAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        if str(qkv.device) == "cpu":
            ...
            q, k, v = torch.split(
                qkv, (self.d * self.nq, self.d * self.nkv, self.d * self.nkv), dim=-1
            )
        ...
        if not packed and y.position_ids is None:
            ...
            if y.cache_seqlens is None:
                y.position_ids = torch.cat(
                    [
                        torch.arange(int(l), dtype=torch.int32, device=y.input_ids.device)
                        for l in lengths
                    ]
                )
                assert (
                    y.position_ids < x.max_seqlen
                ).all() and y.position_ids.max() == x.max_seqlen - 1
            else:
                y.position_ids = torch.cat(
                    [
                        torch.arange(int(l), dtype=torch.int32, device=y.input_ids.device)
...
class FlashMQATForCausalLM(nn.Module):
    ...
    def from_gpt2(
        ...
        config = FlashMQATConfig(
            ...
            intermediate_dim=gpt2config.n_inner
            if gpt2config.n_inner is not None
            else 4 * gpt2config.n_embd,
...
class HuggingfaceLikeFlashMQATForCausalLM(nn.Module):
    ...
    def forward(
        ...
        if packed_input_ids is None and attention_mask is not None:
            build_packed = True
            packed_input_ids, cu_seqlens, max_seqlen = build_packed_inputs(
...
class DeepSpeedChatLikeFlashMQATCriticModel(nn.Module):
    ...
    def from_starcoder(
        ...
        model = cls(
            from_model.net.transformer, output_bias=output_bias, output_scaling=output_scaling
        )
...
def generate(
    ...
    if gconfig.num_samples > 1 and k_caches is None:
        ...
        attention_mask = (
            attention_mask.unsqueeze(1).repeat(1, gconfig.num_samples, 1).flatten(end_dim=1)
        )
    ...
    if k_caches is None:
        ...
        if prompt_padded_len != 1:
            raise ValueError(
                "prompt_padded_len must be 1 when resuming from a previous generation state."
            )
    ...
    def _get_inflight_logits(self) -> torch.FloatTensor:
        ...
        for i in finish_indices:
            ...
            if prompt_tokens is not None:
                ...
                res = dict(
                    prompt=prompt_tokens, gen=gen_tokens, logp=gen_logp, logits_mask=gen_logits_mask
                )
                try:

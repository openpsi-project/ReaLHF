...
actor_model_path="/lustre/public/pretrained_model_weights/sharded_new/Llama-2-7b-hf_4pp_1mp/"
...
reward_model_path=$base_model_path
ref_model_path=$critic_model_path
...
python3 -m apps.quickstart ppo experiment_name=quickstart-debug trial_name=20240227 \
    ...
    ref.base_model_path=$base_model_path \
    ref.parallel.data_parallel_size=1 \
    ref.parallel.pipeline_parallel_size=3 \
    rew.type=llama \
    rew.path=$reward_model_path \
    rew.base_model_path=$base_model_path \
    rew.parallel.data_parallel_size=3 \
    ...
    actor_per_device_train_batch_size=32 \

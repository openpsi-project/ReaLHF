...
def make_dataset(
    cfg: Union[str, reallm.api.core.system.Dataset],
    seed: int,
    ddp_rank: int,
    world_size: int,
    ...
    if isinstance(cfg, str):
        cfg = reallm.api.core.system.Dataset(type_=cfg)

    if isinstance(tokenizer_or_tokenizer_name, str):
        tokenizer = reallm.api.huggingface.load_hf_tokenizer(tokenizer_or_tokenizer_name)
...
def make_dataloader(cfg: Union[str, reallm.api.core.system.DataLoader],
                    dataset: torch.utils.data.Dataset) -> torch.utils.data.DataLoader:
    if isinstance(cfg, str):
        cfg = reallm.api.core.system.DataLoader(type_=cfg)

...
def enumerate_rpc_executions(rpc: ModelRPC, device_mesh: DeviceMesh, num_gen_tokens,
    ...
    for sub_device_mesh in sub_device_meshes:
        ...
        for p in ps:
            ...
            min_bs = (2 * p.num_dp * p.num_pp *
                      n_ppo_minibatches if rpc.interface_type == ModelInterfaceType.TRAIN_STEP else p.num_dp *
                      p.num_pp)
            if min_bs > bs:
            ...
            if p.num_pp > max(device_mesh.n_nodes, 8):
                continue
            mem_cost, static_mem = estimate_rpc_memory(
                rpc,
                p,
                bs,
                seq_len,

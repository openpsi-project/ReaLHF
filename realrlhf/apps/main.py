...
from realrlhf.scheduler.client import JobException, JobState
import realrlhf.api.core.system_api as config_package
import realrlhf.base.constants as constants
import realrlhf.base.logging as logging
import realrlhf.base.name_resolve as name_resolve
import realrlhf.base.names as names
import realrlhf.scheduler.client as sched_client
import realrlhf.system as system
...
def get_repo_path():
    ...
    realrlhf_path = os.path.dirname(os.path.dirname(file_path))
    repo_path = os.path.dirname(realrlhf_path)
    return repo_path
...
def _main_profile_layers(model_family, model_path):
    from realrlhf.api.core.model_api import ModelFamily
    from realrlhf.base.slurm_utils import check_slurm_availability
    from realrlhf.base.testing import clear_name_resolve

    expr_name = trial_name = "profile"
    cmd = (
        f"python3 -m realrlhf.apps.profile_layers --expr_name {expr_name} --trial_name {trial_name} "
    ...
    if check_slurm_availability():
        ...
        from realrlhf.api.core.system_api import _LLM_ENVVARS

...
from realrlhf.api.quickstart.model import ModelTrainEvalConfig
from realrlhf.experiments.common.check import *
from realrlhf.experiments.common.utils import *
from realrlhf.search_engine.search import search_rpc_allocations
...
class CommonExperimentConfig(Experiment):
    ...
    .. code-block:: shell
        ...
        $ python3 -m realrlhf.apps.quickstart sft trial_name=my_trial seed=42 ...
    ...
    def __check_legal_experiment(self):
        ...
        for rpc_name, rpc in self.rpcs.items():
            if not check_is_realrlhf_native_model_interface(
                rpc.interface_impl.type_
            ) and self.allocation_mode in ["search", "heuristic"]:
                raise ValueError(
                    f"RPC {rpc.name} interface is not a realrlhf native implementation. "

...
def remote_worker_cmd(expr_name, trial_name, debug, worker_type):
    ...
    return (
        f"python3 {'' if debug else '-O'} -m realrlhf.apps.remote worker -w {worker_type} "
...
def setup_cmd(expr_name, trial_name, debug):
    bash_cmd = (  # f"pip3 install -e $REAL_PACKAGE_PATH --no-build-isolation && "
        f"python3 {'' if debug else '-O'} -m realrlhf.apps.remote "
...
def control_cmd(
    ...
    bash_cmd = (  # f"pip3 install -e $REAL_PACKAGE_PATH --no-build-isolation && "
        f"python3 {'' if debug else '-O'} -m realrlhf.apps.remote controller "
...
def ray_cluster_cmd(expr_name, trial_name, worker_type):
    ...
    return f"python3 -m realrlhf.apps.remote ray -i {{index}} -g {{count}} {' '.join(flags)}"


def make(mode, expr_name, trial_name, **kwargs) -> SchedulerClient:
    if mode == "slurm":
        from realrlhf.scheduler.slurm.client import SlurmSchedulerClient

        return SlurmSchedulerClient(expr_name, trial_name)
    elif mode == "local":
        from realrlhf.scheduler.local.client import LocalSchedulerClient

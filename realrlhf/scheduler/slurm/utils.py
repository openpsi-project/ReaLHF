...
from realrlhf.base.constants import LOG_ROOT
from realrlhf.scheduler.client import JobException, JobInfo, JobState
...
class SlurmLaunchInfo:
    ...
    def commit(self):
        ...
        # Setup step command.
        # add current directory into container mounts to ensure editable mode for realrlhf package
        container_mounts = (
            f"{os.environ.get('REAL_PACKAGE_PATH', '$PWD')}:/realrlhf,"
...
def get_all_node_resources() -> Dict[str, SlurmResource]:
    ...
    for node in nodes:
        ...
        ctres.gpu_type = atres.gpu_type = (
            realrlhf.base.cluster.spec.gpu_type_from_node_name(node_name)
        )
        rres = ctres - atres
        if rres.valid():

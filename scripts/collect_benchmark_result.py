...
CONFIG_KEYS_TO_PRINT = [
    "actor_model_name", "critic_model_name", "actor_zero_stage", "critic_zero_stage", "hybrid_engine",
    "offload_critic_param", "offload_critic_optimizer_states", "offload_ref", "offload_reward"
]
...
def plot_data(data: dict, dschat_data: dict, fn: str):
    ...
    for dn, (cfg, dp) in data.items():
        ...
        if len(dp["e2e"]) > 0:
            ...
            xticklabels.append(
                f"{dirname_to_n_params(cfg['actor_model_name'])}+{dirname_to_n_params(cfg['critic_model_name'])}"
            )

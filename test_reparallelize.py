...
def _worker(
    ...
    if idx >= 8 - to_topo.world_size():
        to_shard_id = ModelShardID.from_parallelism_rank(
            ModelName("default", 0), to_topo, idx - (8 - to_topo.world_size())
        )
    ...
    if idx < from_topo.world_size():
        ...
        state_dict = {
            i: x[
                from_shard_id.mp_rank * _param_size_per_mp : (from_shard_id.mp_rank + 1) * _param_size_per_mp
            ]
    ...
    for (pp_i, pp_j), layer_indices in repart_strat.items():
        sub_sd = {
            i: (get_param_shape(i, from_topo.get_dim("model")), get_param_dtype(i)) for i in layer_indices
        }

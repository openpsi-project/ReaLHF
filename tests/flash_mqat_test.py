...
#             "/data/aigc/llm/checkpoints/4l-starcoder/")
...
class FlashMQATGPUGPUAccordanceTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        sc_cfg = transformers.AutoConfig.from_pretrained("/lustre/meizy/backup_zy/starcoder")
        # sc_cfg.n_layer = 16
        # sc_cfg.n_embd = 1024
        # sc_cfg.n_head = 8
        # sc_cfg.n_inner = 4096
        ...
        cls.tokenizer = transformers.AutoTokenizer.from_pretrained("/lustre/meizy/backup_zy/starcoder")
        cls.tokenizer.pad_token_id = cls.tokenizer.eos_token_id
        ...
        cls.huggingface_model = starcoder
    ...
    def testGenerate(self):
        ...
        #     "import torch\n",
        # ]
        seqs = [random_sentence(1500, 2500) for _ in range(4)]
        self.tokenizer.padding_side = "left"
        encoding = self.tokenizer(seqs, return_tensors="pt", padding=True)
        prompt: torch.Tensor = encoding['input_ids'].to(self.device)
        prompt_att_mask: torch.Tensor = encoding['attention_mask'].to(self.device)
        ...
        gconfig = GenerationConfig(min_new_tokens=new_token_len,
                                   max_new_tokens=new_token_len,
                                   temperature=1.0,
                                   greedy=True,
                                   top_k=50,
                                   top_p=1.0,
        ...
        vg, vglogprob, vgmask = vanilla_packed_generate(model=self.model,
                                                        ...
                                                        attention_mask=prompt_att_mask,
                                                        gconfig=gconfig)
        print(f"vanilla_packed_generate time {time.monotonic() - st}:")
        print(f"prompts shape: {prompt.shape}")
        ...
        g, logprob, mask, _ = generate(model=self.model,
                                       ...
                                       attention_mask=prompt_att_mask,
                                       gconfig=gconfig)
        print(f"FlashMQAT time {time.monotonic() - st}:")
        print(f"prompts shape: {prompt.shape}")
        print(f"g shape: {g.shape}")
        print(f"g: {g}")

        st = time.monotonic()
        seq = self.huggingface_model.generate(inputs=prompt,
                                              attention_mask=prompt_att_mask,
                                              min_new_tokens=new_token_len,
                                              max_new_tokens=new_token_len)

        print(f"Huggingface time {time.monotonic() - st}:")
        print(f"prompts shape: {prompt.shape}")
        print(f"seq shape: {seq.shape}")
        print(f"extra seq: {seq[:, prompt_len:]}")

...
from tests.pipe_utils import get_example_batch
...
# rank 3: generate outputs: gen_tokens: torch.Size([8, 3])
# tensor([[28375, 13259, 38574],
#         [11841, 25274, 34271],
#         [14372, 14372, 24877],
#         [22157, 34271, 19934],
#         [13259, 46550, 28728],
#         [43657, 17746,   892],
#         [20423, 27744, 17039],
#         [28279, 34271, 19348]], device='cuda:0')
# rank 3: generate outputs: log_probs: torch.Size([8, 3])
# tensor([[-4.8889, -4.2550, -5.9281],
#         [-5.6208, -5.4173, -5.7734],
#         [-5.5537, -4.6465, -5.2243],
#         [-5.9009, -5.1126, -5.8530],
#         [-5.1971, -5.7202, -4.8070],
#         [-5.5349, -5.3346, -5.6072],
#         [-5.9243, -5.5848, -4.9481],
...
class FlashMQATGPUGPUAccordanceTest(unittest.TestCase):
    ...
    def testGenerate(self):
        ...
        # prompt, prompt_att_mask = get_example_batch(self.tokenizer, self.device, 8, 0, 1)
        # self.tokenizer.padding_side = "left"
        encoding = self.tokenizer(seqs, return_tensors="pt", padding=True)
        prompt: torch.Tensor = encoding['input_ids'].to(self.device)
        prompt_att_mask: torch.Tensor = encoding['attention_mask'].to(self.device)

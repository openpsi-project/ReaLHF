...
def test_consistency(model_family_name: str):
    ...
    with constants.model_scope(model_name):
        ...
        logits1 = hf_model(input_ids=input_ids, attention_mask=attention_mask).logits

        logits2 = model(input_ids=input_ids, attention_mask=attention_mask).logits
        assert torch.allclose(logits1, logits2), (logits1 - logits2).abs().max()


if __name__ == "__main__":
    for model_family_name in ["starcoder", "llama"]:
        test_consistency(model_family_name)
